...
class SmsMessageRecipient(Base):
    ...
    phone: Mapped[str] = mapped_column(String(30), nullable=False)
    sms_agree: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    source_labels: Mapped[list[str] | None] = mapped_column(JSON)
    status: Mapped[str] = mapped_column(String(30), default="대기", nullable=False)
...
Index("ix_sms_messages_scheduled_at", SmsMessage.scheduled_at)
Index("ix_sms_messages_scheduled_status", SmsMessage.scheduled_at, SmsMessage.status)
